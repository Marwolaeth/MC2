...
#| label: fig-area-integrand
#| fig-cap: "Illustrating the interpretation of $f(t_0) dt$ as an \"area\"."
...
#| label: fig-bars-0-3
#| fig-cap: "Visualizing the integral $\\int_0^3 f(t) dt$ as the total \"area\" of several $f(t) dt$ bars. The width of each of the bars is $dt$. The height depends on the value of the function $f(t)$ at the bar. For illustration, two of the bars are marked with vertical and horizontal line segments."
#| fig-cap-location: margin
...
```{r echo=FALSE, warning=FALSE}
#| label: fig-neg-area
#| fig-cap: "The $\\int_{-2}^3 g(t) dt$ covers subdomains where $g(t) > 0$ and where $g(t) < 0$. In those latter subdomains, the \"area\" is negative, and shown in light orange here."
raw <- rfun( ~ t, seed=541)
...
@fig-dynamic-anti-d shows a dynamic version of the process of constructing an anti-derivative by Euler steps. The integrand $f(t)$ is shown in the top panel, the anti-derivative $F(t)$ is shown being built up in the bottom panel. The $\color{magenta}{\text{magenta}}$ bar in the top plot is the current Euler step. That step is added to the previously accumulated steps to construct $F(t)$.
...
```{r echo=FALSE}
#| label: fig-dynamic-anti-d
#| fig-cap: "A dynamic view of building $F(t)$ from $f(t)$ by accumulating Euler steps."
knitr::include_graphics("www/area-plot-still.png")
```
...
```{r echo=FALSE}
#| label: fig-sputnik
#| fig-cap: "COVID in Russia. (Source: *The Economist*)"
#| fig-cap-location: margin
...
```{r echo=FALSE}
#| label: fig-sputnik2
#| fig-cap: "Dividing the domain into regions of width $dt = 1$ month."
#| fig-cap-location: margin
...
::: {#tbl-picket-locations .column-margin}
...
`r insert_exercises("Accumulation", "euler")`
