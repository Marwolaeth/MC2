# Local approximations [DRAFT] {#sec-local-approximations}
...
2. Imagine that the grade of the road is described by a positive number for uphill and a negative number for downhill: that is, the slope of the road. For a positive (uphill) grade and at a fixed gear, will the bike's sustained speed be higher or lower as a function of the grade?^[it is much the same for downhill biking, but you have to keep in mind that a shallow downhill has a higher numerical slope than a steep downhill. That is, the derivative of the hill is near zero for a very shallow grade and far from zero (that is, more negative) for a steep downhill grade.]
...
::: {.callout-note icon=false  data-latex=""}
## Why would you want to approximate a function? 
...
In many modeling situations with a single input, you can get very close to a good modeling function $f(x)$ by selecting one of ***eight simple shapes***, shown in @fig-eight-simple-shapes2. 

```{r echo=FALSE}
#| label: fig-eight-simple-shapes2
#| out-width: "40%" 
#| fig-cap: "The ***eight simple shapes***, locally, of functions with one input. (See  Chapter @sec-local-approximations.)"
...
P5 <- slice_plot(-g(x) ~ x, bounds(x=c(-3,-1)), size=2) %>% 
  gf_labs(y="Output", x="Input", title="(E) upward sloping, concave down; steep then shallow")
P6 <- slice_plot(-g(x) ~ x, bounds(x=c(-1,1)), size=2) %>%
...
```{r echo=FALSE}
#| label: fig-eight-simple-shapes3
#| out-width: "40%" 
#| fig-cap: "The ***eight simple shapes***, locally, of functions with one input. (See Chapter @sec-local-approximations.)"
#| fig-cap-location: margin
...
#| fig-cap: "Four of the ***eight simple shapes*** correspond to the sides of the parabola. The labels refer to the graphs in @fig-eight-simple-shapes3."
...
```{r}
#| label: fig-zero-one
#| fig-cap: "The result of our intuitive investigation of the effects of study and tutoring on exam performance. The units are not yet assigned."
...
```{r}
#| label: fig-bodyfat-mod
#| fig-cap: "A low order polynomial model of body fat percentage as a function of height (inches) and weight (lbs)."
...
There is an exercise in DailyDigitals/ 141 DD-35 with some narrative
...
`r insert_exercises("Differentiation", "approximation")`
