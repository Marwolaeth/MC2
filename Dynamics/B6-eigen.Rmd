...
In the previous chapters, you've seen how linear dynamics, when unstable, lead trajectories off to infinity. Chapter @sec-modeling-dynamics looked at some ways that nonlinearity can tame instability, as in the simple models of linear growth.
...
```{r echo=FALSE}
#| label: fig-vec-example-1
#| fig-cap: "The trajectory calculated starting at (-1.1, 2.1). The graph is annotated with the vectors $\\vec{u_1}$ and $\\vec{u_2}$ and the flow field."
...
traj_plot(y(t) ~ x(t), traj, inherit=FALSE, nt=2) %>%
  ...
  flow_field(dx ~ x + y, dy ~ 2*x, domain = bounds(x=-2:2, y=-3:2), color="gray", npts=15) %>%
...
```{r echo=FALSE}
#| label: fig-vec-example-2
#| fig-cap: "Trajectories from the initial conditions $\\color{magenta}{\\vec{u_1}}$ and $\\color{blue}{\\vec{u_2}}$."
#| fig-cap-location: margin
traj_plot(y(t) ~ x(t), traj_u1, bounds(t=0:1),  color="magenta", nt=1) %>%
  traj_plot(y(t) ~ x(t), traj_u2, bounds(t=0:1), color="blue", nt=1) %>%
...
```{r echo=FALSE}
#| label: fig-eigen-flow
#| fig-cap: "The eigenvectors for the $\\left[\\begin{array}{cc}1&1\\\\2&0\\end{array}\\right]$ plotted along with the flow."
#| fig-cap-location: margin
  ...
  flow_field(dx ~ x + y, dy ~ 2*x, domain = bounds(x=-1:1, y=-1:1), color="black", npts=15) %>%
  gf_refine(coord_fixed())
```
