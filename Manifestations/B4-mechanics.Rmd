...
```{r echo=FALSE}
#| column: margin
knitr::include_graphics("www/ramp-decomposition.png")
```
...
```{r echo=FALSE}
#| label: fig-ramp-decomposition
#| fig.show: "keep"
#| fig-cap: "Decomposing the vector of gravitational force into two perpendicular components, one tangent to the ramp and the other perpendicular to it. For clarity, the right triangle of decomposition is shown twice, once without the grid." 
#| column: margin
knitr::include_graphics("www/gravity-right-triangle.png")
```
...
```{r echo=FALSE}
#| column: margin
knitr::include_graphics("www/ramp-triangle.png")
```
...
```{r echo=FALSE}
#| column: margin
knitr::include_graphics("www/curved-ramp.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("www/ramp-segment.png")
```
...
```{r echo=FALSE}
#| label: fig-actin-myosin
#| fig-cap: "Animation of the generation of force by the interaction of actin and myosin, from *The Nervous System in Action*." 
#| out-width: "50%"
#| fig-align: "center" 
knitr::include_graphics("www/actin_myocin_anim.png")
```
...
```{r echo=FALSE}
#| label: fig-jet-engines
#| fig-cap: "Left: A turbojet engine uses air for combustion and emits a relatively low amount of mass at high velocity. Right: A turbofan engine uses a fan blade (1) to convert some of the combustion energy into a large mass of relatively slow velocity, unburnt air."
...
```{r echo=FALSE, warning=FALSE}
#| label: fig-galaxy-probe
#| fig-cap: "An imagined space probe (orange dot) on the outer edges of a galaxy."
#| fig-cap-location: margin
P_galaxy
```
...
```{r echo=FALSE}
#| label: fig-shape1
#| fig-cap: "An irregular shape used in the example. The $(x,y)$ coordinates of closely spaced points on the boundary are available as `Blob1` in the sandbox software."
#| fig-cap-location: margin
P1
```
...
```{r echo=FALSE}
#| label: fig-shape1-blocks
#| fig-cap: "A continuous shape can be approximated by a set of rectangles within the borders of the shape. Integrating over the shape is a matter of adding up across all of the rectangles the relevant quantity for each rectangle."
#| fig-cap-location: margin
P2
```
...
```{r echo=FALSE}
#| label: fig-shape1-cm
#| fig-cap: "For the $y$-component of the center of mass (left panel), the $x$-coordinate of each rectangle is irrelevant. It is as if all the rectangles were moved to $x=0$. Similarly for the $x$-component of the center of mass (right panel)."
#| fig-cap-location: margin
...
```{r echo=FALSE} 
#| label: fig-panel-shape
#| fig-cap: "A panel to locate center of mass"
#| column: margin
top <- rfun(~ x, seed=120)
...
```{r echo=FALSE}
#| label: fig-fun-diff
#| fig-cap: "The panel can be divided into vertical slices, a few of which are shown here. The vertical mid-point of each slice is marked with a blue dash. Accumulating the slices' mid-point coordinates times the slices' mass, and dividing by the panel's mass, gives the $y$-component of the center of mass."
#| fig-cap-location: margin
slices <- tibble(
...
An easier way is to find the vertical center of each slice. For a slice at position $x$, the vertical center is $$y_{\text{mid}}(x) \equiv\left[\strut \text{top}(x) + \text{bottom}(x)\right]/ 2\ ,$$ the average of the top and bottom positions. (These are marked in $\color{blue}{\text{blue}}$ in @fig-fun-diff.)
...
```{r echo=FALSE}
#| label: fig-probe-constant
#| fig-cap: "A space probe accelerating along a linear course. The position at time $t$ can be written as $y(t)$ or as $\theta(t)$."
#| column: margin
knitr::include_graphics("www/rocket-fly-by.png")
```
...
#| label: fig-probe-constant-orig
#| fig-cap: "A space probe (orange) accelerating in a straight line, but observed as a change in angle $\theta$ seen from a planet (green)."
#| column: margin
Arc1 <- tibble(
...
```{r echo=FALSE}
#| label: fig-rocket-on-disk
#| fig-cap: "The space probe mounted on a rigid wheel that can spin around its center."
#| column: margin
knitr::include_graphics("www/rocket-on-disk.png")
```
...
`r insert_exercises("Manifestations", "mechanics")`
