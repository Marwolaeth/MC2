...
```{r echo=FALSE}
#| label: fig-robot-points
#| fig-cap: "The waypoints on a path the robot hand is supposed to follow. All the action is taking place in roughly 1x1 meter area."
...
```{r echo=FALSE}
#| label: fig-piecewise-linear-path
#| fig-cap: "Two functions $x(t)$ and $y(t)$ which describe the path shown in @fig-robot-points."
...
#| label: fig-y-velocity
#| fig-cap: "Velocity versus time time along the path defined by $x(t)$ and $y(t)$ as shown in Figure @fig-piecewise-linear-path."
#| fig-cap-location: margin
...
```{r fig-x-acceleration, echo=FALSE}
ddt_xl <- D(xl(t) ~ t & t, .h=0.1)
ddt_yl <- D(yl(t) ~ t & t, .h=0.1)
P1 <- slice_plot(ddt_xl(t) ~ t, bounds(t=1:16), npts=901) %>% gf_labs(y="x-accel (mm/s^2)", x="")
P2 <- slice_plot(ddt_yl(t) ~ t, bounds(t=1:16), npts=901) %>%
  gf_labs(y="y-accel (mm/s^2)", x = "t (time)")
gridExtra::grid.arrange(P1, P2, nrow=2)
```
...
```{r echo=FALSE}
#| label: fig-ramp-force
#| fig-cap: "A smoothed x-trajectory near station 2. The position of the station is marked with a dot."
#| fig-cap-location: margin
vx <- antiD(ddt_xl(t) ~ t)
xx <- antiD(vx(t) ~ t)
...
```{r echo=FALSE}
#| label: fig-quadratic-spline
#| fig-cap: "Two interpolating functions of the four discrete points (orange). One is piecewise linear (dotted curve), the other is piecewise quadratic (multi-color curve)."
#| fig-cap-location: margin
Pts <- tibble(
  t = 1:4,
  y = c(0, 2, 0.5, 1.7),
...
```{r echo=FALSE}
#| label: fig-four-quads
#| fig-cap: "Four different $C^1$ piecewise quadratic functions that interpolate the knot points. The functions have different values of the derivative at the right end of the domain."
#| fig-cap-location: margin
...
```{r echo=FALSE}
#| label: fig-qspline-x
#| fig-cap: "A quadratic spline interpolation of the $x$-coordinates of the robot-path knots. The data hardly speak for themselves, since the interpolationg function tends to alternate between concave up and concave down in adjacent segments between the knots."
#| fig-cap-location: margin
...
```{r echo=FALSE}
#| label: fig-quad-robot-points-quad
#| fig-cap: "Connecting the robot-path knots with a piecewise quadratic polynomial, constructed to be $C^1$. The path is pretty perhaps, but absurd."
#| fig-cap-location: margin
nudges <- rep(40, 16)
nudges[c(3,12)] <- -40
gf_path(y ~ x, data = Path) %>%
  gf_point(y ~ x, size=8, data = Robot_stations, color="orange", alpha=.5) %>%
  gf_text(y ~ x, label= ~ paste0("t=", t), alpha=0.5, data = Robot_stations, nudge_y=nudges) %>%
  gf_refine(coord_fixed()) %>%
  gf_labs(x="x (mm)", y="y (mm)")
```

Quadratic splines are rarely used in practice. (A ***cubic spline*** provides helpful flexibility. See Chapter @sec-cubic-splines.) In @fig-qspline-x you can see one of the reasons: the quadratic form is so ***stiff*** that the interpolating function tends to shift from concave up to concave down (or vice versa) at each knot point. This results in the interpolating function tending to have a local minimum or maximum *between* adjacent knots, even if the data themselves to not indicate such a structure.
...
```{r echo=FALSE}
#| label: fig-smooth-robot-points
#| fig-cap: "Connecting the robot-path knots with a piecewise cubic polynomial, constructed to be $C^2$. This is a much smoother  path than produced by interpolation with quadratic polynomials."
#| fig-cap-location: margin 
nudges <- rep(40, 16)
nudges[c(3,12)] <- -40
gf_path(y ~ x, data = Path) %>%
...
```{r echo=FALSE}
#| label: fig-cubic-spline-x
#| fig-cap: "A cubic spline interpolation of the $x$-coordinates of the robot-path knots. The cubic spline respects the monotonicity of consecutive knot points."
#| fig-cap-location: margin
slice_plot(xfun(t) ~ t, bounds(t=1:16), npts=500) %>%
  gf_point(x ~ t, data = Robot_stations, color="orange", alpha=0.5, size=4) %>%
  gf_labs(y="x(t)")
...
<iframe width="600" height="337" src="https://www.youtubetrimmer.com/view/?v=SgJcj2Gd0Rc&start=695&end=800&loop=0" title="Motion without interpolation" frameborder="0" allow="accelerometer;  clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
```{r echo=FALSE}
#| label: fig-letter-e
#| fig-cap: "The outline of a letter in a computer font is often specified by a series of knot points (red dots). The path passes smoothly through some of the knot points, but has a discontinuous derivative at others."
#| column: margin
...
```{r echo=FALSE}
#| label: fig-bezier-1
#| fig-cap: "A single Bézier segment is defined by two endpoints and two control points. Drag the control points to see how the shape of the curve is defined by them."
#| out-width: "1%"
...
```{r echo=FALSE}
#| label: fig-bezier-2
#| fig-cap: "Two Bézier segments can be arranged in to create a smooth or non-smooth junction between them." 
#| out-width: "1%"
knitr::include_graphics("www/tiny-blank.png")
```
...
```{r echo=FALSE}
#| label: fig-bezier-3
#| fig-cap: "A Bézier curve leaves each endpoint in a direction that is tangent to the line drawn between the endpoint and its control point."
#| out-width: "1%"
knitr::include_graphics("www/tiny-blank.png")
```
...
`r insert_exercises("Manifestations", "data-driven functions")`
