...
The variety of shapes of the nine pattern-book functions means that, often, one or another will be suitable for the modeling situation in hand. [Combining the functions to create a greater diversity of shapes is the subject of Chapter @sec-assembling.]{.aside}
...
#| label: fig-covid-exp
#| fig-cap: "Cumulative officially confirmed COVID-19 cases (in 10000s) during the month of March, 2020. The $\\color{magenta}{\\text{magenta curve}}$ is $e^t$."
...
```{r echo=FALSE}
#| column: margin
knitr::include_graphics("www/thermometer.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("www/ruler.jpeg")
```
...
```{r echo=FALSE}
#| label: fig-tides-ri1
#| fig-cap: "Tide levels oscillate up and down over time. This is analogous to the $\\sin(t)$ pattern-book function."
#| fig-cap-location: margin
...
```{r  echo=FALSE}
#| label: fig-scaling-nature
#| fig-cap: "Natural **quantities** must be scaled to pure numbers before being suited to the pattern-book functions. The output from the pattern-book function is a pure number which is scaled to the natural **quantity** of interest."
#| column: page-right
#| out-width: "150%"
...
::: {#tbl-parameterization .column-screen-inset-right}
...
```{r child="Exercises/Drill-modeling-01.Rmd"}
```
