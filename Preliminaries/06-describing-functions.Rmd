...
Pa <- slice_plot(exp(x) ~ x, bounds(x=c(-3,3))) %>%
...
Psinusoid <- slice_plot(sin(x) ~ x, bounds(x=c(-5,5))) %>%
  ...
  gf_hline(yintercept=~0, color="magenta", linetype="dashed")
Pc <- slice_plot(1/x ~ x, bounds(x=c(.01, 1.25))) %>%
  slice_plot(1/x ~ x, bounds(x=c(-1.25, -.01))) %>%
...
#| fig-cap-location: margin
Segs <- Znotes::create_segments(sin(x) ~ x, bounds(x=c(-5,5)), nsegs=100) %>%
  mutate(y2end = 2.15*yend)

Small <- Segs[seq(2,nrow(Segs), by=3),]
slice_plot(sin(x) ~ x, bounds(x=c(-5, 5))) %>%
gf_segment(yf + yfend ~ x + xend, 
           data = Small, 
           color=~slope,
           alpha=0.65, size=3, lineend="round") %>%
...
Referring to the three function examples in @fig-for-words, we will use the traditional terms ***concave up*** and ***concave down*** to refer to positive and negative concavity respectively.
...
```{r child="Exercises/Drill-preliminaries-06.Rmd"}
```
